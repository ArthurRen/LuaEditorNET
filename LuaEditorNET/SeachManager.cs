...
namespace CodeEditorNet.Lua
{
    ...
    {
        public static ScintillaNET.Scintilla TextArea = null;
        ...
        public static void Find(bool next, bool incremental)
        {
            ...
            if (LastSearch.Length > 0)
            {
                ...
                else
                {
                    ...
                    // Search, and if not found..
                    int searchPos = -1;
                    if (TextArea.CurrentPosition == TextArea.SelectionEnd)
                    {
                        searchPos = TextArea.CurrentPosition - LastSearch.Length;
                    }
                    else
                    {
                        searchPos = TextArea.CurrentPosition;
                    }
                    if (searchPos < 0)
                    {
                        //MessageBox.Show("已经搜索到顶点！");
                        return;
                    }
                    string strAim = TextArea.Text.Substring(0, searchPos);
                    int indexStart = TextArea.Text.LastIndexOf(LastSearch, searchPos, searchPos + 1);
                    if (indexStart == -1)//TextArea.SearchInTarget(LastSearch) == -1)
                    {
                        //MessageBox.Show("已经搜索到顶点！");
                    ...
                    {
                        TextArea.SetSelection(indexStart + LastSearch.Length, indexStart);
                        TextArea.ScrollCaret();
                        LastSearchIndex = TextArea.TargetStart;
                    }
                    return;
